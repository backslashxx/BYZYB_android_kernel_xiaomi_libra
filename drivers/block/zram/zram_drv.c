...
static void zram_page_end_io(struct bio *bio, int bi_error)
{
	struct page *page = bio->bi_io_vec[0].bv_page;

	page_endio(page, bio_data_dir(bio), bi_error);
	bio_put(bio);
}
			...
			unsigned long entry, struct bio *parent)
{
	...
	bio->bi_sector = entry * (PAGE_SIZE >> 9);
...
static DEVICE_ATTR(debug_stat, S_IRUGO, debug_stat_show, NULL);
...
static DEVICE_ATTR(compact, S_IRUGO, NULL, compact_store);
...
#ifdef CONFIG_ZRAM_WRITEBACK
static DEVICE_ATTR(backing_dev, S_IRUGO | S_IWUSR,
